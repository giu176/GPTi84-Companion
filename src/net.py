"""Pico W wifi + framed-TCP client for the phone relay chat bridge.

Loads SSID/PSK and SERVER_* from `secrets`. Frames are 4-byte big-endian
length followed by N payload bytes. The intended production peer is the
companion app running on the user's phone, reachable over the same Wi-Fi
network or the phone's hotspot.

The bridge layer treats the socket as a simple byte stream:
send_framed() writes [u32_be length][payload], and FrameReader polls the
same shape for replies from the phone.
"""
...
def _sync_clock():
    ...
    Failure is non-fatal for the phone relay's raw TCP path."""
    try:
        import ntptime
    except ImportError:
...
def open_socket(host=None, port=None):
    """Open the phone relay framed-TCP socket."""
